...
## Several stacks on one Docker network

Some deploy platforms (e.g. Dokploy) attach every stack to one shared Docker network.
There the internal hostname `api` resolves to every container of that name across all
stacks, so web, worker, and bot would reach other projects' containers. Set
`SERVICE_URL_API` in `.env` to this stack's unique container name, e.g.
`SERVICE_URL_API=http://<project>-api-1:7002`.
