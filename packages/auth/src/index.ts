...
// otherwise derive it from the first frontend origin that yields a parent domain —
// an apex entry listed before a subdomain must not suppress the derivation.
const cookieDomain =
  process.env.COOKIE_DOMAIN || trustedOrigins.map((origin) => parentDomain(origin)).find(Boolean);
